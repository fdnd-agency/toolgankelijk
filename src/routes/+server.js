    ...
    const formData = await request.formData();
            ...
            (async () => {
                try {
                    ...
                    await sendUpdate({ status: 'Start sitemap check' });

                    // 2) Sitemap / URL-extractie
                    if (toggleSitemap) {
                        // check if url ends with a /
                        url = url.endsWith('/') ? url : url + '/';
                        await sendUpdate({ status: 'Formatting url' });
                        ...
                        const sitemapPromises = sitemapArray.map((sitemapPath) => {
                            return new Promise(async (resolve, reject) => {
                                try {
                                    ...
                                    await sendUpdate({ status: `Testing the path: ${url + sitemapPath}` });
                        ...
                        try {
                            ...
                            if (urlArray.length === 0) {
                                ...
                                await sendUpdate({ status: 'Sitemap niet gevonden, probeer de urls van de pagina te extraheren' });
